...
def test_cmake_toolchain(toolchain):
    ...
    build_dir = Path(__file__).parent/"build"/os.environ["CMAKE_ENV"]/toolchain
    if build_dir.exists():
        shutil.rmtree(build_dir)
    app_dir = Path(__file__).parent/"test_app"
    run(["cmake", "--version"], check=True)
    
    proc = run(["cmake", "-B", str(build_dir), "-S", str(app_dir), f"-DCMAKE_TOOLCHAIN_FILE={TOOLCHAIN.format(toolchain)}"],
               text=True, stdout=PIPE, stderr=PIPE)
    check(proc)

    proc = run(["cmake", "--build", "."], cwd=str(build_dir), text=True, stdout=PIPE, stderr=PIPE)
    check(proc)
